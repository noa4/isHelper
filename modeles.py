...
class BesoinsMgr():
    ...
    def update(self, besoin):
        if (isinstance(besoin, Besoin)):
            self.cursor.execute(
                """UPDATE besoins (intitule,primaire) VALUES (:intitule, :primaire) WHERE id_besoin = :id_besoin""",
                {id_besoin: besoin.id_besoin(), intitule: besoin.nom(), primaire: besoin.primaire()})
...
class PieceMgr():
    ...
    def update(self, piece):
        if (isinstance(piece, Piece)):
            self.cursor.execute(
                """UPDATE pieces (nom_piece,couleur) VALUES (:nom_piece,:couleur) WHERE id_piece = :id_piece""",
                {id_piece: piece.id(), nom_piece: piece.nom(), couleur: piece.couleur()})
